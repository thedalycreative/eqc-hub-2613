...
const CLASSES = [
	...
	},
	{
		...
		title: 'Pushing to GitHub + polish', sub: 'Commit, push, QA pass',
		focus: 'Picking up where Class 12 ran out of time: getting your code back <em>up</em> to GitHub. Then a polish pass on the clone.',
		topics: [
			'Finish any leftover Git setup — everyone needs <code>git --version</code> and <code>gh --version</code> to answer',
			'Create a blank repo, then attach your project to it',
			'The push loop: Source Control → write a message → Commit → Sync Changes',
			'The same thing in the terminal: <code>git add .</code> → <code>git commit -m "..."</code> → <code>git push</code>',
			'Then polish: alt text on every image, consistent spacing, test on a real phone',
